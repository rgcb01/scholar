...
public final class MinecraftTextMeasurer implements TextMeasurer {
    ...
    public int measureWidth(String text, TextStyle style) {
        ...
        return Math.max(0, Math.round(typographyResolver.logicalWidth(
                typographyResolver.component(text, resolved, style)) * widthScale(style)));
    }

    @Override
    public int lineHeight(TextStyle style) {
        var resolved = typographyResolver.resolve(style);
        return typographyResolver.typography().documentLineHeight(style,
                typographyResolver.fontFor(resolved.role()).lineHeight,
                resolved.lineHeightAdjustment());
    }

    @Override
    public TextCaretMetrics caretMetrics(TextStyle style, int lineHeight) {
        return typographyResolver.typography().documentCaretMetrics(style, lineHeight);
    }

    private static float widthScale(TextStyle style) {
